...
/**
 * Tests for home-ground advantage by training a classifier with the difference between the 
 * distance-travelled measure for each team, plus a constant.
 */
public class TrainHomeTeamAdvantage5 {
	...
	public static void main(String[] args) {
		...
		getTrainer(trainingData, testingData).train(false, 1000);
	...
	private static GoldData getMatchData(final Collection<Match> matches) {
		return new GoldMatchDataNoDraws(matches) {
			...
			protected DataVector getFeatures(Match match) {
				...
				return VectorFactory.newVector(1.0, homeDist - awayDist);
			}
		};
	}
