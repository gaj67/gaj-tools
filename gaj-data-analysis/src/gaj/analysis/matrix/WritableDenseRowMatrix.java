...
/*package-private*/ class WritableDenseRowMatrix extends AbstractMatrix<WritableVector> implements DenseMatrix, WritableMatrix, RowMatrix {
	...
	/*package-private*/ WritableDenseRowMatrix(double[][] data) {
		super(data.length, data[0].length);
		this.data = data;
	}

	// XXX Shared data, not writable?
	/*package-private*/ WritableDenseRowMatrix(DenseMatrix matrix) {
		super(matrix.numRows(), matrix.numColumns());
		WritableDenseRowMatrix dMat = (WritableDenseRowMatrix) matrix;
		this.data = dMat.data;
	}

	/*package-private*/ WritableDenseRowMatrix(int numRows, int numColumns) {
