...
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, rnode/.style={draw,rectangle,minimum size=3em,inner sep=3pt}]
    ...
    \draw[->] (1) edge node [pos=0.5, above] {$b_2=0$} (3) ;
    \draw[->] (1) edge node [pos=0.5, above] {$b_2=1$} (4) ;
    \draw[->] (4) edge node [pos=0.5, above] {$b_3=0$} (5) ;
    \draw[->] (4) edge node [pos=0.5, above] {$b_3=1$} (6) ;
\end{tikzpicture}
...
\begin{algorithm}[hbt]
 ...
 \BlankLine
 \tcp{Initialise queue}${\cal Q}\leftarrow []$\;
 Obtain root node: $\eta\leftarrow\textbf{get\_root}({\cal T}_{\cal V})$\;
 Construct score node: $\nu\leftarrow\texttt{make\_node}(\texttt{node}=\eta, \texttt{path}=(),\texttt{score}=1)$\;
 Insert score node: $Q\leftarrow\texttt{insert}({\cal Q}, \nu)$\;
 \While{$|{\cal Q}|>0$}{
  Obtain highest-scoring node: $\nu\leftarrow\textbf{pop}({\cal Q})$\;
  ...
  \eIf{$\texttt{is-leaf}(\eta)$}{
   Obtain word: $w\leftarrow\texttt{get\_word}(\eta)$\;
   \KwRet{$(w,s,\rho)$}\;
  }{
   Compute probability: $p\leftarrow\texttt{get\_prob}(\eta, {\bf x})$\;
