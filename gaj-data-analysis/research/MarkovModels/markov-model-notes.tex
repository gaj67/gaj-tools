...
\subsection{Extended Forward--Backward Algorithm}
...
\begin{eqnarray}
   p(\underline{\iota},\vec{s},\vec{x},\underline{\tau}\,|\,\theta) 
& = & 
   \sum_{i_t'=1}^{S}\delta(i_t'\!=\!i_t)\,p(\underline{\iota},\vec{s}_{t-1},\vec{x}_t,S_t=\sigma_{i'_t}\,|\,\theta) 
\,p(\rvec{s}_{t+1},\rvec{x}_{t+1},\underline{\tau}\,|\,S_t=\sigma_{i'_t},\theta) 
\nonumber\\& = &
  \sum_{i=1}^{S}\delta(i\!=\!i_t)\,\alpha_{t,i}\,\beta_{t,i}\,.
\end{eqnarray}
The forward step $\alpha_{t,i}$ is recursively defined as
\begin{eqnarray}
   \alpha_{t,i} & = & p(\underline{\iota},\vec{s}_{t-1},\vec{x}_t,S_t=\sigma_{i}\,|\,\theta) 
\nonumber\\& = &
   \sum_{j=1}^{S}\delta(j=i_{t-1})\,p(\underline{\iota},\vec{s}_{t-2},\vec{x}_{t-1},S_{t-1}=\sigma_{j}\,|\,\theta) 
      \,p(\tau_t\!=\!0\,|\,S_{t-1}\!=\!\sigma_j,\theta)
\nonumber\\&&
    \hspace*{5mm}{}\times\,p(S_t\!=\!\sigma_i\,|\,\tau_{t}\!=\!0,S_{t-1}\!=\!\sigma_j,\theta)
\,p(X_t\!=\!x_t\,|\,S_t\!=\!\sigma_i,\theta)
\nonumber\\& = &
   \left\{\sum_{j=1}^{S}\delta(j=i_{t-1})\,\alpha_{t-1,j}\,\omega_{0,t-1,j}\,\Gamma_{t-1,j,i}\right\}\,o_{t,i}(x_t)\,,
\end{eqnarray}
for $t=2,3,\ldots,n$. The forward pass commences with the first step
\begin{eqnarray}
  \alpha_{1,i} & = & p(\underline{\iota},x_1,S_1=\sigma_{i}\,|\,\theta) 
\nonumber\\& = & 
...
\,.
\end{eqnarray}
Observe that if we substitute $\alpha_{1,j}$ into $\alpha_{2,i}$, then the result is just
the entire summation over $i'_1$ from the forward model~\eqref{eq:forward} multiplied by
the next term $o_{2,i}(x_2)$ from the $i_2'$ summation.
Also note that the standard forward pass derivation commences with $\pi_{*,1,i}$ and does not include the
$\delta(\cdot)$ or $\omega$ terms.

Conversely to the forward pass, the backward step $\beta_{t,i}$ is recursively defined as
\begin{eqnarray}
   \beta_{t,i} & = & p(\rvec{s}_{t+1},\rvec{x}_{t+1},\underline{\tau}\,|\,S_t\!=\!\sigma_{i},\theta) 
\nonumber\\& = & 
   \sum_{j=1}^{S}\delta(j\!=\!i_{t+1})\,
p(\rvec{s}_{t+2},\rvec{x}_{t+2},\underline{\tau}\,|\,S_{t+1}\!=\!\sigma_{j},\theta) 
\,p(X_{t+1}=x_{t+1}\,|\,S_{t+1}\!=\!\sigma_j,\theta)
\nonumber\\&& \hspace*{5mm}{}\times
       p(S_{t+1}\!=\!\sigma_j\,|\,\tau_{t+1}\!=\!0,S_t\!=\!\sigma_i,\theta)
\,p(\tau_{t+1}\!=\!0\,|\,S_t\!=\!\sigma_i,\theta)
\nonumber\\& = & 
	\left\{\sum_{j=1}^{S}\delta(j\!=\!i_{t+1})\,\beta_{t+1,j}\,o_{t+1,j}(x_{t+1})\,\Gamma_{t,i,j}\right\}
\,\omega_{0,t,i}
\,,
\end{eqnarray}
...
\end{eqnarray}
Observe that if we substitute $\beta_{n,j}$ into $\beta_{n-1,i}$, then the result is just
the entire summation over $i'_n$ from the backward model~\eqref{eq:backward} multiplied by the previous term
$\omega_{0,n-1,i'_{n-1}}$ from the $i'_{n-1}$ summation.
