...
\end{eqnarray}
for $n\ge m$ and $n\ge\ell-1$.

It is more usual to further restrict the stateful process depicted in Figure~\ref{fig:stateful-sub}
...
\end{figure}
The corresponding order-$(1,1)$ sequence model is now given by
\begin{eqnarray}
p(\iota_0,\vs,\vx,\tau_{n+1}) & = &
...
define $p(*\,|\,\tm_t,s_t)=\sum_{x_t\in{\cal X}}p(X_t\!=\!x_t\,|\,\tau_t\!=\!0,S_t\!=\!s_t)=1$; hence $p(\underline{x}_t\,|\,\tm_t,s_t)$ is well defined. Note that this simplification is only applicable when
...
$p(*\,|\,\tm_n,s_n)=\sum_{\tau_{n+1}=0}^{1} p(\tau_{n+1}\,|\,\tau_n\!=\!0,S_n\!=\!s_n)=1$.
