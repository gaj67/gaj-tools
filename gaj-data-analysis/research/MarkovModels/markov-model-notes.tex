...
\nonumber\\
& = & p({\iota},\bar{\tau}_1,r_1,\ldots,\bar{\tau}_n,r_n,{\tau})\,.
\end{eqnarray}
...
p({\iota})
\,p(\bar{\tau}_1\,|\,{\iota})
\,p(r_1\,|\,{\iota},\bar{\tau}_1)
\,p(\bar{\tau}_2\,|\,{\iota},\bar{\tau}_1,r_1)
\, p(r_2\,|\,{\iota},\bar{\tau}_1,r_1,\bar{\tau}_2)
\nonumber\\
&&
\cdots
p(r_n\,|\,{\iota},\ldots,\bar{\tau}_{n})
\, p({\tau}\:|\;{\iota},\ldots,r_n)
\,.
...
& = &
p({\iota})
\left\{\prod_{t=1}^{n}p(\bar{\tau}_t\,|\,{\iota},\vec{\bar{\tau}}_{t-1},\vec{r}_{t-1})
\,p(r_t\,|\,{\iota},\vec{\bar{\tau}}_t,\vec{r}_{t-1})\right\}
 p({\tau}\:|\;{\iota},\vec{\bar{\tau}}_n,\vec{r}_n)
\,.
...
\nonumber\\
& = &
p({\iota})\,p(\bar{\tau}_1\,|\,{\iota})
\,p(r_1\,|\,{\iota},\bar{\tau}_1)
\left\{\prod_{t=2}^{n}p(\bar{\tau}_t\,|\,r_{t-1})
...
\nonumber\\&&\hspace*{10mm}{}\times\left.\strut
...
&=&
p({\iota})\,p(\bar{\tau}_1\,|\,{\iota})
\,p(s_1\,|\,{\iota},\bar{\tau}_1)
\,p(x_1\,|\,s_1,{\iota},\bar{\tau}_1)
\nonumber\\&&
{}\times\left\{\prod_{t=2}^{n}p(\bar{\tau}_t\,|\,s_{t-1},x_{t-1})
...
\,p(x_1\,|\,s_1)
\nonumber\\&&
{}\times\left\{\prod_{t=2}^{n}p(\bar{\tau}_t\,|\,s_{t-1})
...
Similarly, if the termination marker $\tau_{n+1}$ is unobserved, then we write $\underline{\tau}=*$ to indicate maginalisation over
$\tau_{n+1}\in\{0,1\}$,  with the consequence that
$p(\tau_{n+1}=*\,|\,S_n)=1$ and hence $p(\underline{\tau}\,|\,s_n)$ is well defined.
However, if the initiation marker $\iota_0$ is unobserved, i.e.\ $\underline{\iota}=*$ from above, then the marginalisation is more difficult
because $\iota_0$ also conditions $S_1$ via $p(S_1\,|\,\iota_0,\tau_1)$. To allow for this, we note that
\begin{eqnarray}
 p(\iota,\bar{\tau}_1,s_1) & = & p(\iota_0=\iota)\,p(\tau_1=0\,|\,\iota_0=\iota)\,p(S_1=s_1\,|\,\iota_0=\iota,\tau_1=0)\,,
\end{eqnarray}
and so define
\begin{eqnarray}
 p(\underline{\iota},\bar{\tau}_1,s_1) & = & \sum_{\iota=0}^{1}\delta(\iota=\underline{\iota}) \, p(\iota,\bar{\tau}_1,s_1)\,,
\end{eqnarray}
...
p(\underline{\iota},\bar{\tau}_1,s_1)
\,p(\underline{x}_1\,|\,s_1)
%\nonumber\\&&{}\times
\left\{\prod_{t=2}^{n}p(\bar{\tau}_t\,|\,s_{t-1})
\,p(s_t\,|\,\bar{\tau}_t,s_{t-1})\,p(\underline{x}_t\,|\,s_t)\right\}
\,p(\underline{\tau}\,|\,s_n)
\,.
