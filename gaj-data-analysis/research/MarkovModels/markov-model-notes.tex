...
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, onode/.style={fill=black!50, draw,circle,minimum size=1em}]
   ...
   \end{scope}
\end{tikzpicture}
\caption{\em A 2nd-order Markov process for generating complete, random sequences of random length $N$.}
...
Then, for the initial stages $t<m$, for $m>1$, the forward factor~\eqref{eq:fwd-pass-basic} becomes
\begin{eqnarray}
...
Similarly, for  stages $t\ge m$
the backward factor~\eqref{eq:bwd-pass-basic} becomes
\begin{eqnarray}
\beta_t(\vr_{t-m+1,t}) & = & 
...
\,.
\end{eqnarray}
Furthermore, if $t>m$ then we may move backward to stage $t-1$ to obtain the recursive relationship
\begin{eqnarray}
...
\begin{eqnarray}
\beta_t(\vr_{1,t}) & = & 
p(\tm_{t+1},\ur_{t+1},\ldots,\tm_n,\ur_n,\ut\,|\,\iota_0,\tm_{1},r_{1},\ldots,\tm_t,r_t)\,,
\end{eqnarray}

In general, the backward pass is defined via
\begin{eqnarray}
\beta_{t-1}(\vr_{t-m,t-1}) & = & 
\sum_{r_{t}\in{\cal R}} \delta(r_{t}\!=\!\ur_{t})\,
p_m^*(\tm_{t},r_{t}\,|\,\iota_0,\vec{\tau}^-_{1,t-1},\vr_{1,t-1})\,\beta_{t}(\vr_{t-m+1,t})
\,.
\end{eqnarray}
For the special case of $m=1$, the backward pass reduces to
\begin{eqnarray}
...
\begin{eqnarray}
 p(\iota,\tm_1,s_1) & = & p(\iota_0=\iota)\,p(\tau_1=0\,|\,\iota_0=\iota)\,p(S_1=s_1\,|\,\iota_0=\iota,\tau_1=0)\,,
\end{eqnarray}
and so define
\begin{eqnarray}
 p(\ui,\tm_1,s_1) & = & \sum_{\iota=0}^{1}\delta(\iota\!=\!\ui) \, p(\iota,\tm_1,s_1)\,,
\end{eqnarray}
...
%\nonumber\\&&{}\times
\left\{\prod_{t=2}^{n}p(\tm_t\,|\,s_{t-1})
\,p(s_t\,|\,\tm_t,s_{t-1})\,p(\underline{x}_t\,|\,s_t)\right\}
\,p(\ut\,|\,s_n)
\,.
