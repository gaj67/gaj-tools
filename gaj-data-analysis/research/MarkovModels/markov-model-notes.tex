...
Similarly, we may denote $\iota_t=1$ simply by $\iota_t$, on the understanding that $\bar{\iota}_t$ denotes the negation $\iota_t=0$.
Likewise, we may denote $\tau_t=1$ by just $\tau_t$ and $\tau_t=0$ by $\bar{\tau}_t$.
Hence, it is plausible to simplify equation~\eqref{eq:p_r_complete} as
\begin{eqnarray}
p(\langle\vec{r}\rangle) 
& = & p(\iota_0,\bar{\tau}_1,r_1,\ldots,\bar{\tau}_n,r_n,\tau_{n+1})\,.
\end{eqnarray}
In general, we may handle both complete and partial sequences by letting $\underline{\iota}$ denote the observed start-of-sequence symbol, and
letting $\underline{\tau}$ denote the observed end-of-sequence symbol; hence
\begin{eqnarray}
p(\underline{\iota},\vec{r},\underline{\tau})
& = & p(\iota_0\!=\!\underline{\iota},\tau_1\!=\!0,R_1\!=\!r_1,\ldots,\tau_n\!=\!0,R_n\!=\!r_n,\tau_{n+1}\!=\!\underline{\tau})
\nonumber\\
& = & p(\underline{\iota},\bar{\tau}_1,r_1,\ldots,\bar{\tau}_n,r_n,\underline{\tau})\,.
\end{eqnarray}
For convenience, we let $\underline{\iota}=*$ if we do not know whether or not $r_1$ is the start of the sequence,
and similarly let $\underline{\tau}=*$ if we do not kow whether or not $r_n$ is the end of the sequence.
The `*' symbol is just a convenience -- its presence has no effect on the computed probabilities, and merely indicates that the true value
of $\iota_0$ and/or $\tau_{n+1}$ should be marginalised out. Thus, for example:
\begin{eqnarray}
p(\vec{r}) 
& = & p(*,\bar{\tau}_1,r_1,\ldots,\bar{\tau}_n,r_n,*)~=~p(\bar{\tau}_1,r_1,\ldots,\bar{\tau}_n,r_n)\,.
\end{eqnarray}
Finally, we may utilise this notation of convenience to simplify the explicitly terminated process of Figure~\ref{fig:random-process} 
to more resemble the implicitly terminatedl process of Figure~\ref{fig:R-process}; the result is shown in Figure~\ref{fig:simple-random-process}.
\begin{figure}[hbt]
\centering
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, onode/.style={fill=black!50, draw,circle,minimum size=1em}]
    ...
    \draw[->] (0) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_1}{}$}  (1) ;
    \draw[->] (1) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_2}{}$}  (2) ;
    \draw[->] (2) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_3}{}$}  (t) ;
    \draw[->] (t) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_N}{}$}  (3) ;
    \draw[->] (3) edge node [pos=0.5, above] {$\stackrel{\tau_{N+1}}{}$} (tt) ;
\end{tikzpicture}
...
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, onode/.style={fill=black!50, draw,circle,minimum size=1em}]
    ...
    \draw[->] (0) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_1}{}$}  (1) ;
    \draw[->] (1) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_2}{}$}  (2) ;
    \draw[->] (2) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_3}{}$}  (3) ;
    \draw[->] (3) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_4}{}$}  (t) ;
    \draw[->] (t) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_N}{}$}  (N) ;
    \draw[->] (N) edge node [pos=0.5, above] {$\stackrel{\tau_{N+1}}{}$} (tt) ;
...
\cdots
p(R_n\!=\!r_n\,|\,\iota_0\!=\!\underline{\iota},\ldots,\tau_{n}\!=\!0)
\nonumber\\&&
{}\times p(\tau_{n+1}\!=\!\underline{\tau}\:|\;\iota_0\!=\!\underline{\iota},\ldots,R_n\!=\!r_n)
\nonumber\\
& = &
p(\underline{\iota})
\,p(\bar{\tau}_1\,|\,\underline{\iota})
\,p(r_1\,|\,\underline{\iota},\bar{\tau}_1)
\,p(\bar{\tau}_2\,|\,\underline{\iota},\bar{\tau}_1,r_1)
\, p(r_2\,|\,\underline{\iota},\bar{\tau}_1,r_1,\bar{\tau}_2)
\nonumber\\
&&
\cdots
p(r_n\,|\,\underline{\iota},\ldots,\bar{\tau}_{n})
\, p(\underline{\tau}\:|\;\underline{\iota},\ldots,r_n)
\,.
...
\prod_{t=1}^{n}\left\{p(\tau_t\!=\!0\,|\,\iota_0\!=\!\underline{\iota},\vec{\tau}_{t-1}\!=\!\vec{0},
...
\nonumber\\&&
{}\times p(\tau_{n+1}\!=\!\underline{\tau}\:|\;\iota_0\!=\!\underline{\iota},\vec{\tau}_n\!=\!\vec{0},\vec{R}_n\!=\!\vec{r}_n)
\nonumber\\
& = &
p(\underline{\iota})
\prod_{t=1}^{n}\left\{p(\bar{\tau}_t\,|\,\underline{\iota},\vec{\bar{\tau}}_{t-1},\vec{r}_{t-1})
\,p(r_t\,|\,\underline{\iota},\vec{\bar{\tau}}_t,\vec{r}_{t-1})\right\}
 p(\underline{\tau}\:|\;\underline{\iota},\vec{\bar{\tau}}_n,\vec{r}_n)
\,.
...
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, onode/.style={fill=black!50, draw,circle,minimum size=1em}]
    ...
    \draw[->] (0) edge node [pos=0.6, above] {$\stackrel{\bar{\tau}_1}{}$}  (1) ;
    \draw[->] (1) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_2}{}$}  (2) ;
    \draw[->] (2) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_3}{}$}  (3) ;
    \draw[->] (3) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_4}{}$}  (t) ;
    \draw[->] (t) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_{N-1}}{}$}  (Nm1) ;
    \draw[->] (Nm1) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_N}{}$}  (N) ;
    \draw[->] (N) edge node [pos=0.4, above] {$\stackrel{\tau_{N+1}}{}$}  (tt) ;
...
\nonumber\\
& = &
p(\underline{\iota})\,p(\bar{\tau}_1\,|\,\underline{\iota})
\,p(r_1\,|\,\underline{\iota},\bar{\tau}_1)
\prod_{t=2}^{n}\left\{p(\bar{\tau}_t\,|\,r_{t-1})
\,p(r_t\,|\,\bar{\tau}_t,r_{t-1})\right\}
\,p(\underline{\tau}\:|\;r_n)
\,.
\end{eqnarray}
...
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, onode/.style={fill=black!50, draw,circle,minimum size=1em}]
    ...
    \draw[->] (2) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_3}{}$}  (3) ;
    \draw[->] (3) edge node [pos=0.5, above] {$\stackrel{\bar{\tau}_4}{}$}  (t) ;
    \draw[->] (t) edge node [pos=0.7, above left] {$\stackrel{\bar{\tau}_N}{}$} (N) ;
    \draw[->] (N) edge node [pos=0.75, above left] {$\stackrel{\tau_{N+1}}{}$} (tt) ;
...
\nonumber\\&&{}\times
\,p(X_1\!=\!x_1\,|\,S_1\!=\!s_1,\iota_0\!=\!\underline{\iota},\tau_1\!=\!0)
\nonumber\\&&
...
\nonumber\\&&
{}\times p(\tau_{n+1}\!=\!\underline{\tau}\,|\,S_n\!=\!s_n,X_n\!=\!x_n)
\nonumber\\
&=&
p(\underline{\iota})\,p(\bar{\tau}_1\,|\,\underline{\iota})
\,p(s_1\,|\,\underline{\iota},\bar{\tau}_1)
\,p(x_1\,|\,s_1,\underline{\iota},\bar{\tau}_1)
\nonumber\\&&
{}\times\prod_{t=2}^{n}\left\{p(\bar{\tau}_t\,|\,s_{t-1},x_{t-1})
\,p(s_t\,|\,\bar{\tau}_t,s_{t-1},x_{t-1})
\,p(x_t\,|\,s_t,s_{t-1},x_{t-1})\right\}
\nonumber\\&&
{}\times p(\underline{\tau}\,|\,s_n,x_n)
\,.
\end{eqnarray}
...
\;p(\tau_{n+1}\!=\!\underline{\tau}\,|\,S_n\!=\!s_n)
\nonumber\\
&=&
p(\underline{\iota})\,p(\bar{\tau}_1\,|\,\underline{\iota})
\,p(s_1\,|\,\underline{\iota},\bar{\tau}_1)
\,p(x_1\,|\,s_1)
\nonumber\\&&
{}\times\prod_{t=2}^{n}\left\{p(\bar{\tau}_t\,|\,s_{t-1})
\,p(s_t\,|\,\bar{\tau}_t,s_{t-1})\,p(x_t\,|\,s_t)\right\}
\,p(\underline{\tau}\,|\,s_n)
\,.
