...
In practice, the dependence of $S_t$ and $X_t$ on $X_{t-1}$ (and further ancestral values) induces a regression model of some complexity,
...
Finally, as we discussed earlier, the general `spirit' of  stateful processes is that the state drives the value, not the other way around.
Hence, common practice is to limit all dependencies to a maximum of $m$ previous {\em nodes}, as shown in Figure~\ref{fig:stateful-m}.
\begin{figure}[hbt]
\centering
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, onode/.style={fill=black!50, draw,circle,minimum size=1em}]
    \node (0i) at (-0.7,0) [right] {$\stackrel{\iota_0^+}{}$};
...
\end{tikzpicture}
\caption{\em An example ($m=2$) of an $m$th-order stateful Markov process for generating complete, random sequences of random length $N$ consisting of pairs of states and values.}
\label{fig:stateful-m}
\end{figure}
