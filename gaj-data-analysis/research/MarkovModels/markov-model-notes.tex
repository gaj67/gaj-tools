...
p(\iota_0,\vs,\vx,\tau_{n+1}) & = &
p(\iota_0)
\left\{\prod_{t=1}^{m}p(\tm_t,s_t\,|\,\iota_0,\vec{\tau}^-_{1,t-1},\vs_{1,t-1})\right\}
\left\{\prod_{t=1}^{\ell-1}p(x_t\,|\,\iota_0,\vec{\tau}^-_{1,t},\vs_{1,t})\right\}
\nonumber\\&&
\times
\left\{\prod_{t=m+1}^{n}p(\tm_t,s_t\,|\,\vec{\tau}^-_{t-m,t-1},\vs_{t-m,t-1})\right\}
\left\{\prod_{t=\ell}^{n}p(x_t\,|\,\vec{\tau}^-_{t-\ell+1,t},\vs_{t-\ell+1,t})\right\}
\nonumber\\&&
\times
p(\tau_{n+1}\,|\,\vec{\tau}^-_{n-m+1,n},\vs_{n-m+1,n})\,,
...
\sum_{\iota_0=0}^{1}\delta(\iota_0\!=\!\ui)
\left\{
p(\iota_0)\prod_{t=1}^{m}p(\tm_t,s_t\,|\,\iota_0,\vec{\tau}^-_{1,t-1},\vs_{1,t-1})
\right\}\right.
\nonumber\\&&\times
\left.\left\{
\prod_{t=1}^{\ell-1}
\sum_{x_t\in{\cal X}} \delta(x_t\!=\!\ux_t)\,
p(x_t\,|\,\iota_0,\vec{\tau}^-_{1,t},\vs_{1,t})
\right\}\right]
\nonumber\\&&\times
\left\{\prod_{t=m+1}^{n}p(\tm_t,s_t\,|\,\vec{\tau}^-_{t-m,t-1},\vs_{t-m,t-1})\right\}
\nonumber\\&&\times
...
\prod_{t=\ell}^{n}
\sum_{x_t\in{\cal X}} \delta(x_t\!=\!\ux_t)\,
p(x_t\,|\,\vec{\tau}^-_{t-\ell+1,t},\vs_{t-\ell+1,t})
\right\}
\nonumber\\&&\times
\left\{
\sum_{\tau_{n+1}=0}^{1}\delta(\tau_{n+1}\!=\!\ut)\,
...
\begin{eqnarray}
p(\underline{x}_t\,|\,\iota_0,\vec{\tau}^-_{1,t},\vs_{1,t}) & = & 
\sum_{x_t\in{\cal X}} \delta(x_t\!=\!\ux_t)\,
p(x_t\,|\,\iota_0,\vec{\tau}^-_{1,t},\vs_{1,t})\,,
\\
p(\underline{x}_t\,|\,\vec{\tau}^-_{t-\ell+1,t},\vs_{t-\ell+1,t}) & = &
\sum_{x_t\in{\cal X}} \delta(x_t\!=\!\ux_t)\,
p(x_t\,|\,\vec{\tau}^-_{t-\ell+1,t},\vs_{t-\ell+1,t})\,,
\end{eqnarray}
to deduce that
...
\begin{eqnarray}
p(\ut\,|\,\vec{\tau}^-_{n-m+1,n},\vs_{n-m+1,n}) & = & 
\sum_{\tau_{n+1}=0}^{1}\delta(\tau_{n+1}\!=\!\ut)\,
p(\tau_{n+1}\,|\,\vec{\tau}^-_{n-m+1,n},\vs_{n-m+1,n})\,,
\end{eqnarray}
or more precisely
\begin{eqnarray}
p(\tau_{n+1}\!=\!\ut\,|\,\vec{\tau}^-_{n-m+1,n},\vs_{n-m+1,n}) & = & 
\sum_{\tau=0}^{1}\delta(\tau\!=\!\ut)\,
p(\tau_{n+1}\!=\!\tau\,|\,\vec{\tau}^-_{n-m+1,n},\vs_{n-m+1,n})\,,
\end{eqnarray}
is also well defined, since likewise  $p(\tau_{n+1}\!=\!*\,|\,\ldots)=\sum_{\tau=0}^{1} p(\tau_{n+1}\!=\!\tau\,|\,\ldots)=1$.
...
\begin{eqnarray}
p(\iota_0,\vec{\tau}^-_{1,m},\vs_{1,m}) & = & p(\iota_0)\prod_{t=1}^{m}p(\tm_t,s_t\,|\,\iota_0,\vec{\tau}^-_{1,t-1},\vs_{1,t-1})
\end{eqnarray}
and
\begin{eqnarray}
p(\iota_0,\vec{\tau}^-_{1,m},\vs_{1,m},\uvx_{1,\ell-1}) & = & 
p(\iota_0,\vec{\tau}^-_{1,m},\vs_{1,m})
\prod_{t=1}^{\ell-1}p(\ux_t\,|\,\iota_0,\vec{\tau}^-_{1,t},\vs_{1,t})
\,,
\end{eqnarray}
since $\ell-1\le m$. Hence we may first compute the marginal probabilities
\begin{eqnarray}
p(\ui,\vec{\tau}^-_{1,m},\vs_{1,m},\uvx_{1,\ell-1}) & = &
\sum_{\iota_0=0}^{1}\delta(\iota_0\!=\!\ui)\,
p(\iota_0,\vec{\tau}^-_{1,m},\vs_{1,m},\uvx_{1,\ell-1})
\end{eqnarray}
and then the joint probabilities
\begin{eqnarray}
p(\ui,\vs,\uvx,\ut) & = &
p(\ui,\vec{\tau}^-_{1,m},\vs_{1,m},\uvx_{1,\ell-1})
\left\{\prod_{t=m+1}^{n}p(\tm_t,s_t\,|\,\vec{\tau}^-_{t-m,t-1},\vs_{t-m,t-1})\right\}
\nonumber\\&&\times
\left\{\prod_{t=\ell}^{n}p(\ux_t\,|\,\vec{\tau}^-_{t-\ell+1,t},\vs_{t-\ell+1,t})\right\}
\,p(\ut\,|\,\vec{\tau}^-_{n-m+1,n},\vs_{n-m+1,n})
\end{eqnarray}
to finally arrive at the (hidden) state marginalisation
\begin{eqnarray}
p(\ui,\uvs,\uvx,\ut) & = & 
\sum_{s_1\in{\cal S}} \delta(s_1\!=\!\us_1)
\cdots\sum_{s_n\in{\cal S}} \delta(s_n\!=\!\us_n)
...
Under the conventions discussed in the previous section for handling missing observations, the forward--backward algorithm for order--$(m,\ell)$ stateful Markov sequences takes the form
