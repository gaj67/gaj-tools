...
in scope to some arbitray {\em historical} stage $s$, as also shown.
\begin{figure}[hbt]
\centering
\begin{tikzpicture}[cnode/.style={draw,circle,minimum size=3em,inner sep=3pt}, onode/.style={fill=black!50, draw,circle,minimum size=1em}]
    ...
    \draw[->] (0) edge node [pos=0.5, above] {$\stackrel{\tau^-_1}{}$}  (dots1) ;
    \draw[->] (dots1) edge node [pos=0.5, above] {$\stackrel{\tau^-_s}{}$}  (s) ;
    \draw[->] (s) edge node [pos=0.5, above] {$\stackrel{\tau^-_{s+1}}{}$}  (dots2) ;
    \draw[->] (dots2) edge node [pos=0.5, above] {$\stackrel{\tau^-_t}{}$}  (t) ;
    \draw[->] (t) edge node [pos=0.5, above] {$\stackrel{\tau^-_{t+1}}{}$}  (dots3) ;
    \draw[->] (dots3) edge node [pos=0.5, above] {$\stackrel{\tau^-_{N}}{}$} (N) ;
...
\end{tikzpicture}
\caption{\em Causality allows the sequence to be partitioned at some pivot stage $t$. Further
limitation of past dependencies to some historical stage $s$ defines the active region for
...
where
\begin{eqnarray}
\alpha_t(\vr_{s,t}) & = & p(\ui,\tau_1^-,\ur_1,\ldots,\tau^-_{s-1},\ur_{s-1},
\tau^-_s,r_s,\ldots,\tau^-_t,r_t)
\label{eq:fwd-pass}
\end{eqnarray}
...
\begin{eqnarray}
\beta_t(\vr_{s,t}) & = & 
p(\tau^-_{t+1},\ur_{t+1},\ldots,\tau^-_n,\ur_n,\ut\,|\,\ui,\tau_1^-,\ur_1,\ldots,\tau^-_{s-1},\ur_{s-1},
\tau^-_s,r_s,\ldots,\tau^-_t,r_t)\
\label{eq:bwd-pass}
\end{eqnarray}
...
\end{eqnarray}
We can generalise this model by first defining a forward observation sub-sequence $\vr_t=(r_1,r_2,\ldots,r_t)$,
for $t=1,2,\ldots,n$, where $\vr_0$ is taken to be the empty sub-sequence by definition.
...
\vec{R}_{t-1}\!=\!\vr_{t-1})\right.
\nonumber\\&&
\hspace*{20mm}{}\times \left.\rule{0pt}{6mm}
p(R_t\!=\!r_t\,|\,\iota_0\!=\!{\iota},\vec{\tau}_t\!=\!\vec{0},\vec{R}_{t-1}\!=\!\vr_{t-1})\right\}
\nonumber\\&&
{}\times p(\tau_{n+1}\!=\!{\tau}\:|\;\iota_0\!=\!{\iota},\vec{\tau}_n\!=\!\vec{0},\vec{R}_n\!=\!\vr_n)
\nonumber\\
& = &
p({\iota})
\left\{\prod_{t=1}^{n}p(\tau^-_t\,|\,{\iota},\vec{\tau^-}_{t-1},\vr_{t-1})
\,p(r_t\,|\,{\iota},\vec{\tau^-}_t,\vr_{t-1})\right\}
 p({\tau}\:|\;{\iota},\vec{\tau^-}_n,\vr_n)
\,.
